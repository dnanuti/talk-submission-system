...
public class TalkController {
    ...
    public TalkResponse createDraft(@Valid @RequestBody TalkRequest request) {
        return TalkResponse.from(submitTalkUseCase.createDraft(
                new CreateDraftCommand(request.title(), request.abstractText(), request.speakerName())));
    }
    ...
    public TalkResponse addManualReview(@PathVariable Long id, @Valid @RequestBody ManualReviewRequest request) {
        return TalkResponse.from(reviewTalkUseCase.addManualReview(
                new AddManualReviewCommand(id, request.reviewer(), request.feedback(), request.approved())));
    }

    @PostMapping("/{id}/accept")
    public TalkResponse accept(@PathVariable Long id) {
        return TalkResponse.from(reviewTalkUseCase.accept(id));
    }

    @PostMapping("/{id}/reject")
    public TalkResponse reject(@PathVariable Long id, @RequestBody RejectRequest request) {
        return TalkResponse.from(reviewTalkUseCase.reject(id, request.reason()));
    }
